...
for alpha in alpha_vec: 
    ...
    for i in range(k):
        ...
        # Step 5: Test the model on the test data and record the score
    ...
    # parts = []
    # for i in range(k):
        # print((i%k)*(len(sample_train)*0.2), ((i+1)%k)*(len(sample_train)*0.2))
        # if(i == k-1):
        #     test = (sample_train[(i%k)*(len(sample_train)*0.2):])
        # else:
        #     test = (sample_train[(i%k)*(len(sample_train)*0.2):((i+1)%k)*(len(sample_train)*0.2)])
        
        # if(i == k-2):
        #     validation = sample_train[((i+2)%k)*(len(sample_train)*0.2):((i+1)%k)*(len(sample_train)*0.2)]
        # else:
        #     validation = sample_train[((i+1)%k)*(len(sample_train)*0.2):((i+2)%k)*(len(sample_train)*0.2)]
        
        # if(i >= k-3):        
        #     train = sample_train[((i+3)%k)*(len(sample_train)*0.2):(i%k)*(len(sample_train)*0.2)]
        # else:
        #     train = sample_train[(i%k)*(len(sample_train)*0.2):((i+3)%k)*(len(sample_train)*0.2)]
        ...
        # print((i%k)*(len(sample_train)*0.2), ((i+1)%k)*(len(sample_train)*0.2))
        # test = sample_train[(i%k)*(len(sample_train)*0.2), ((i+1)%k)*(len(sample_train)*0.2)]
        # print(((i+1)%k)*(len(sample_train)*0.2), len(sample_train)-((i+2)%k)*(len(sample_train)*0.2))
        # validation = sample_train[((i+1)%k)*(len(sample_train)*0.2), len(sample_train)-((i+2)%k)*(len(sample_train)*0.2)]
